# Source review: `official-learn-evaluation`

## Snapshot

- Source: Evaluation
- Canonical URL: https://learn.microsoft.com/en-us/agent-framework/agents/evaluation
- Source class: `official-doc`
- Owner/author: Microsoft Learn / Microsoft
- Reviewed page date: 2026-05-27
...
- MAF topics: evaluation.
- Target evidence: `docs/decisions/0023-foundry-evals-integration.md`; `python/packages/core/agent_framework/_evaluation.py`; `dotnet/tests/Microsoft.Agents.AI.UnitTests/EvaluationTests.cs`; evaluation samples.
- Version/maturity: Core evaluation APIs are current at the live page but cloud evaluators require Foundry resources; exact exports remain version-sensitive.
...
- Important correspondence or drift: Target `__init__.pyi` exports evaluation helpers, but live page additions must be checked against target symbols. Cloud evaluation was not run.

## Design and implementation value

- Reusable decisions: Strong design route for local deterministic checks, expected outputs/tool calls, repetitions, conversation splitters, workflow evaluation, and optional cloud judges.
...
- Prohibited shortcut: Do not equate one LLM-judge score with release readiness; define dataset, baseline, threshold, repetitions, failure preservation, cost, and privacy.
...
- State: `adopted`
- Approved use: primary official explanation and routing source for **evaluation**, always paired with target revision/package coordinates and matching source/tests.
- Prohibited or unsafe use: Do not equate one LLM-judge score with release readiness; define dataset, baseline, threshold, repetitions, failure preservation, cost, and privacy.
- Topic-registry promotion proposal: route under `evaluation`; preserve all additional topic tags from `sources.json`.
