# Source review: `official-learn-agui`

## Snapshot

- Source: AG-UI Integration with Agent Framework
- Canonical URL: https://learn.microsoft.com/en-us/agent-framework/integrations/ag-ui/
...
- Languages: C#, Python, TypeScript frontend, and separately maintained Go SDK.
- MAF topics: protocols, approvals-hitl, security.
- Target evidence: `docs/decisions/0010-ag-ui-support.md`; `dotnet/samples/02-agents/AGUI`; Python AG-UI packages/samples; protocol tests under `dotnet/tests` and `python/packages`.
- Version/maturity: Protocol integration is current but endpoint, event, state, tool-rendering, and frontend packages must be version-paired.
...
- Important correspondence or drift: Target C#/Python evidence supports the integration, while TypeScript is a consumer boundary and Go is external. Client/server protocol versions must match.

## Design and implementation value

- Reusable decisions: Canonical UI protocol route for streaming, backend/frontend tools, shared state, HITL, workflow exposure, testing, and MCP Apps.
...
- Prohibited shortcut: Do not let frontend state or tool results bypass server authorization; protect approvals against replay/forgery and validate all client events.
...
- State: `adopted`
- Approved use: primary official explanation and routing source for **protocols, approvals-hitl, security**, always paired with target revision/package coordinates and matching source/tests.
- Prohibited or unsafe use: Do not let frontend state or tool results bypass server authorization; protect approvals against replay/forgery and validate all client events.
