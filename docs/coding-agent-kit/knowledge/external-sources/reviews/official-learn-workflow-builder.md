# Source review: `official-learn-workflow-builder`

## Snapshot

- Source: Workflow Builder & Execution
- Canonical URL: https://learn.microsoft.com/en-us/agent-framework/workflows/workflows
...
- Languages: C# and Python; separately maintained Go SDK may appear.
- MAF topics: workflows.
- Target evidence: `python/packages/core/agent_framework/_workflows`; `dotnet/src/Microsoft.Agents.AI.Workflows`; `dotnet/tests/Microsoft.Agents.AI.Workflows.UnitTests`.
- Version/maturity: Graph builder is current; exact builder methods and event types are language/version specific.
...
- Important correspondence or drift: The target engines provide the graph/runtime seam. The documented superstep barrier is material: parallel branches can block advancement and must not be modeled as independent task queues.

## Design and implementation value

- Reusable decisions: Explains graph construction, execution, streaming/non-streaming results, and bulk-synchronous superstep barriers.
...
- Prohibited shortcut: Do not infer race freedom inside an executor, exactly-once delivery, unlimited fan-out, or production persistence from builder examples.
...
- Prohibited or unsafe use: Do not infer race freedom inside an executor, exactly-once delivery, unlimited fan-out, or production persistence from builder examples.
